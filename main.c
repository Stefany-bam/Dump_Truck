...
double BL;
double BS;
float Average_Loader_Utilization;       //Average Loader Utilization of both the loaders
float Average_Scale_Utilization;        //Average Scale Utilization of the Weighing Scale
...
void main()
{
	...
	printf("6 dump trucks are used to haul material X from our company to the industry.\nThere are two loaders to load material X into the dump trucks.\nA single weighing scale is then used to weigh the loaded trucks.\n\n");
	...
	while(clock <= 80 && fel_start != NULL)		//Printing the Future Event List till clock time is 80
	{
	...
	BS=wbusy_time;
	CT=clock;
    Average_Loader_Utilization = ((BL / 2)/ CT)*100;
    Average_Scale_Utilization = (BS / CT)*100;
    printf("\n\t\tAverage Loader Utilization: \t%.2f ", Average_Loader_Utilization ); printf("%%");
    printf("\n\t\tAverage Scale Utilization:\t%.2f ", Average_Scale_Utilization); printf("%%");
    printf("\n");
}
